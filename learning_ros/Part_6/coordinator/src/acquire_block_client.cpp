...
int main(int argc, char** argv) {
   ...
   while(1)
   {
		...
		desired_pose.pose.position.x = g_result.object_pose.pose.position.x;
		desired_pose.pose.position.y = g_result.object_pose.pose.position.y;
		desired_pose.pose.position.z = g_result.object_pose.pose.position.z;
		desired_pose.pose.orientation.x = 0.707*(1-a) + g_result.object_pose.pose.orientation.x*a;
		desired_pose.pose.orientation.y = 0.707*(1-a) + g_result.object_pose.pose.orientation.y*a;
		desired_pose.pose.orientation.z = 0*(1-a) + g_result.object_pose.pose.orientation.z*a;
		desired_pose.pose.orientation.w = 0*(1-a) + g_result.object_pose.pose.orientation.w*a;
		ROS_INFO("HEY: %f %f %f %f", g_result.object_pose.pose.orientation.x, g_result.object_pose.pose.orientation.y, g_result.object_pose.pose.orientation.z, g_result.object_pose.pose.orientation.w);
		desired_pose.header.stamp = ros::Time::now();  
		
		double major_length;
		nh.getParam("/bml", major_length);
		
		if (major_length > 0.045 && major_length < 0.093)
		{
			ROS_INFO("ITS A RECTANGULAR BLOCK");
			desired_pose.pose.position.y = desired_pose.pose.position.y + 0.25;
		}
		else if (major_length > 0.019 && major_length < 0.045)
		{
			ROS_INFO("ITS A CUBE BLOCK");
			desired_pose.pose.position.y = desired_pose.pose.position.y - 0.25;
		}
		else
		{
			ROS_INFO("ITS EITHER TOO SMALL OR TOO BIG TO BE EITHER BLOCK");
		}
