...
bool PclUtils::find_plane_fit(double x_min, double x_max, double y_min, double y_max, double z_min, double z_max, double dz_tol,
    ...
    float offset_angle = q.dot(major_axis); 
    	
    for (size_t i = 0; i < cloud_filtered->points.size (); ++i)
	{
		cloud_filtered2->push_back(cloud_filtered->points[i]);
	}
	
	ROS_INFO("Offset Angle: %f", offset_angle);

	for (size_t i = 0; i < cloud_filtered2->points.size (); ++i)
	{
		if (i%20 == 0)
		{
			cloud_filtered2->points[i].x = cos(offset_angle)*cloud_filtered2->points[i].x + sin(offset_angle)*cloud_filtered2->points[i].x;
			cloud_filtered2->points[i].y = -sin(offset_angle)*cloud_filtered2->points[i].y + cos(offset_angle)*cloud_filtered2->points[i].y;
		}
	}
	...
	for (size_t i = 0; i < cloud_filtered2->points.size (); ++i)
	{
		if (i%20 == 0)
		{
			if (cloud_filtered2->points[i].x < min_x_pt)
			{
				min_x_pt = cloud_filtered2->points[i].x;
			}
			if (cloud_filtered2->points[i].x > max_x_pt)
			{
				max_x_pt = cloud_filtered2->points[i].x;
			}
			
			if (cloud_filtered2->points[i].y < min_y_pt)
			{
				min_y_pt = cloud_filtered2->points[i].y;
			}
			if (cloud_filtered2->points[i].y > max_y_pt)
			{
				max_y_pt = cloud_filtered2->points[i].y;
			}
		}
	}
    
	ROS_INFO("X Length: %f, Y Length: %f, Offset Angle: %f", max_x_pt-min_x_pt, max_y_pt-min_y_pt, offset_angle);
	
	float major_length;
	
	if ((max_x_pt-min_x_pt) > (max_y_pt-min_y_pt))
	{
		major_length = max_x_pt-min_x_pt;
	}
	else
	{
		major_length = max_y_pt-min_y_pt;
	}
	
	ros::NodeHandle nh;
	nh.setParam("/bml", major_length);
